...
export default function ClubHub() {
  ...
  async function handleJoin(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (clubError || !club) {
      alert('Invalid Club ID. Please check the code and try again.');
      return;
    }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 mt-8 relative">
      <div className="flex flex-col items-center justify-center mb-12 text-center gap-4">
        ...
        <Link href="/club/create" className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 px-6 rounded-md transition shadow-lg shadow-blue-900/20 mt-2 inline-block">
          + Create Club
        </Link>
      </div>
        ...
        <>
          {user && (
            <div className="max-w-xl mx-auto mb-16 bg-gray-800/50 p-6 rounded-xl border border-gray-700 backdrop-blur-sm">
              ...
              <form onSubmit={handleJoin} className="flex gap-2">
                <input type="text" name="clubId" required placeholder="Paste Unique Club ID here..." className="flex-1 bg-gray-900 border border-gray-600 text-white rounded-md p-3 focus:ring-2 focus:ring-blue-500 outline-none text-sm font-mono placeholder:font-sans" />
                <button type="submit" className="bg-gray-700 hover:bg-gray-600 text-white font-bold py-3 px-6 rounded-md transition border border-gray-600">Join</button>
              </form>
            </div>
          )}
          ...
          {!user ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {myClubs.map((club) => (
                <Link key={club.id} href={`/club/${club.id}`} className="block bg-gray-800 p-6 rounded-xl border border-gray-700 hover:border-blue-500 hover:shadow-2xl hover:shadow-blue-900/20 transition group relative overflow-hidden">
