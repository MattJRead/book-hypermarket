...
export default async function ClubHub() {
  ...
  if (user) {
    ...
    if (data) {
      myClubs = data.map(membership => membership.clubs);
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 mt-8 relative">
      ...
      {/* 1. Header with Top-Right Create Button */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-4">
        <div>
          <h1 className="text-4xl font-bold text-white mb-2">Book Club Hub</h1>
        ...
        {user && (
          <Link 
            href="/club/create" 
            className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 px-6 rounded-md transition shadow-lg shadow-blue-900/20"
          >
            + Create Club
          </Link>
        )}
      ...
      {/* 2. Centered Search Bar for Joining */}
      {user && (
