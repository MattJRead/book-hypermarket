    ...
    <>
      <div className={`fixed bottom-28 left-1/2 -translate-x-1/2 z-40 w-64 rounded-2xl shadow-2xl border overflow-hidden transition-all duration-300 ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'} ${isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'}`}>
        <div className="flex flex-col">
          ...
          {userId ? (
            <>
              <Link href="/bookshelf" className={`px-6 py-4 font-bold text-center border-b transition-colors ${isDarkMode ? 'border-gray-800 text-sky-400 hover:bg-gray-800' : 'border-gray-100 text-sky-600 hover:bg-gray-50'}`}>
                📚 My Bookshelf
              </Link>
              <Link href="/wishlist" className={`px-6 py-4 font-bold text-center border-b transition-colors ${isDarkMode ? 'border-gray-800 text-emerald-400 hover:bg-gray-800' : 'border-gray-100 text-emerald-600 hover:bg-gray-50'}`}>
                🎯 My Wishlist
              </Link>
              <Link href="/account" className={`px-6 py-4 font-bold text-center border-b transition-colors ${isDarkMode ? 'border-gray-800 text-white hover:bg-gray-800' : 'border-gray-100 text-gray-900 hover:bg-gray-50'}`}>
                ⚙️ My Account
              </Link>
            </>
          ) : (
          ...
          {userId && (
            <button onClick={async () => { await supabase.auth.signOut(); window.location.href='/'; }} className={`px-6 py-4 font-bold text-center text-red-400 transition-colors ${isDarkMode ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}>
              Sign Out
            </button> 
          )}
        </div>
