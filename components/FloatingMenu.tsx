...
export default function FloatingMenu() {
  ...
  const changeTheme = (newTheme: string) => {
    ...
    // Broadcast the change to the rest of the website instantly
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-3">
        {isOpen && (
          <div className="flex flex-col w-[300px] gap-3 max-h-[75vh] overflow-y-auto px-2 pb-2 scrollbar-hide">
            ...
            <div className="flex flex-col gap-2 w-full">
              <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="theme-element w-full px-6 py-4 rounded-full font-bold text-center border flex justify-between items-center">
                <span>Profile</span>
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="theme-element w-16 h-16 rounded-full border-2 flex items-center justify-center transition-transform hover:scale-105 z-50">
          {isOpen ? <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"></path></svg> : <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 6h16M4 12h16M4 18h16"></path></svg>}
        </button>
      </div>
    </>
  );
